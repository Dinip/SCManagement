...
namespace SCManagement.Services.NotificationService
{
    public class NotificationService : INotificationService
    {
        ...
        public void NotifyPaymentLate(int payId)
        {
            _backgroundWorker.Enqueue(async (_serviceProvider) =>
            {
                using var scope = _serviceProvider.CreateScope();
                ...
                var payment = await _context.Payment.Include(p => p.Product).FirstOrDefaultAsync(f => f.Id == payId);

                List<string> strings = new List<string> { payment.UserId };
                ...
                {
                    { "_USERNAME_", $"{user.FullName}"},
                    { "_ID_", $"{_hostUrl}/Payment?payId={payment.Id}"},
            ...
            _backgroundWorker.Enqueue(async (_serviceProvider) =>
            {
                ...
                foreach (var subscription in subs)
                {
                    ...
                    Dictionary<string, string> values = new Dictionary<string, string>
                    {
                        ...
                        { "_FREQUENCY_", _sharedResource.Get(subscription.Frequency.ToString(), user.Language) },
                        { "_VALUE_",$"{subscription.Value}€" },
                        { "_CLUBNAME_", subscription.Club.Name },
                    };
